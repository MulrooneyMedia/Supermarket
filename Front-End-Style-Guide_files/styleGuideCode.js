// Temporary home for JavaScript code while in development within the Style Guide 
//alert("styleGuideID start");
/**
...
JS.base = new (function(){
    ...
    this.init = function (){
        ...
        switch (JS.pageName){
        case 'styleGuideID':
            // Call code to make carousels work. 
            JS.objects.carousel = new JS.CarouselScroller({wrapperID:"carousel2",contentTarget:"carouselContent2"});       
            //JS.objects.carousel3 = new JS.CarouselScroller({wrapperID:"carousel3",contentTarget:"carouselContent3"});
...
JS.CarouselScroller = function(options) {
    ...
    */
    ...
    /**
     * Stores the event handler that waist for the CSS3 transition animation to finish.
     * @type Object
     * @private
     */
    ...
    var _init = function (){
        ...
        if (_wrapperElem && _contentElem) {
            ...
            // Listen out for swiping actions
            _swipingCarousel(_contentElem);
        }
        else {
    ...
    /**
     * Listen out for swiping actions on the products within the carousel
     * and react accordingly.
     * @private
     */
    var _swipingCarousel = function (_contentElem){

        /* HammerJS version for swiping */
        var hammerOptions = {
            swipeVelocityX: 0.2
        };
        var hammerInstance = new Hammer(_contentElem, hammerOptions);

        hammerInstance.on('swiperight', function(ev) {
            //console.log("Swipe occured right");//!!
            _carouselJumpRight(_anchorLeftElem);
        });
        hammerInstance.on('swipeleft', function(ev) {
            //console.log("Swipe occured left");//!!
            _carouselJumpLeft(_anchorRightElem);
        });
        

        /* Dojo version
            // This function needs the Gesture - Swipe library
            require(["dojox/gesture/swipe","dojo/on"],
                function(swipe,on){
                    //console.log("_swipingCarousel");//!!

                    // React to a swipe
                    dojo.connect(_contentElem,swipe.end,function(e){
                        //console.log("Swipe occured");//!!
                        // Check which difference is bigger (x or y) since only supporting left and right swipes
